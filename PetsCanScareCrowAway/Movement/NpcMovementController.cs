...
namespace PurrplingCore.Movement
{
    ...
    /// </summary>
    public class NpcMovementController : Internal.IUpdateable
    {
        ...
        public bool AcquireTarget(Vector2 targetTile)
        {
            ...
            this.pathToFollow = this.pathFinder.Pathfind(this.follower.Tile, targetTile);
            this.timeout = this.BlockedTimer;

            if (this.pathToFollow != null && this.pathToFollow.Count > 0 && this.follower.Tile != this.pathToFollow.Peek())
            {
        ...
        {
            string footStepSound = Utility.isOnScreen(this.follower.Tile, 1) ? "Cowboy_Footstep" : "";
            if (dir < 0 || dir > 3)
                return;
        ...
        private void HandleGates()
        {
            if (this.gatesInThisLocation && (this.follower.xVelocity != 0 || this.follower.yVelocity != 0))
            {
                ...
                Vector2 tile = this.follower.Tile;
